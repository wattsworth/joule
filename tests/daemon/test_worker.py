...
MODULE_ECHO_DATA = os.path.join(os.path.dirname(
    __file__), 'worker_scripts', 'echo_data.py')
MODULE_RUN_FOREVER = os.path.join(os.path.dirname(
    __file__), 'worker_scripts', 'run_forever.py')
...
class TestWorker(asynctest.TestCase):
    ...
    def setUp(self):
        ...
        # mock up a stream with int32_4 data format so numpypipe builds
        # correctly
        self.myprocdb.find_stream_by_path = \
