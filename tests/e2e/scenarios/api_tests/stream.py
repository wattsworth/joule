...
class TestStreamMethods(asynctest.TestCase):
    ...
    async def test_stream_move_to_existing_folder(self):
        await self.node.stream_move("/test/f1/f1a/s1a1", "/test/f2/f2a")
        stream = await self.node.stream_get("/test/f2/f2a/s1a1")
        self.assertEqual("s1a1", stream.name)

    async def test_stream_move_to_new_folder(self):
        await self.node.stream_move("/test/f1/f1a/s1a1", "/test/f2/new")
        stream = await self.node.stream_get("/test/f2/new/s1a1")
        self.assertEqual("s1a1", stream.name)

    async def test_move_errors(self):
        # stream does not exist
        with self.assertRaises(errors.ApiError):
            await self.node.stream_move("/test/f1/f1a/bad", "/test/f2/new")

        # name conflict
        stream = api.Stream(name="s1a1", elements=[api.Element(name="e1")])
        await self.node.stream_create(stream, "/test/f2")

        with self.assertRaises(errors.ApiError):
            await self.node.stream_move(stream, "/test/f1/f1a")

        # locked stream
        with self.assertRaises(errors.ApiError):
            await self.node.stream_move("/live/base", "/test/f2/new")

    async def test_stream_update(self):
        stream = await self.node.stream_get("/test/f1/f1a/s1a1")
        stream.name = "new name"
        stream.description = "new description"
        stream.elements[0].name = "new element name"
        await self.node.stream_update(stream)
        stream = await self.node.stream_get(stream)
    ...
    async def testStreamUpdateErrors(self):
        # locked stream
        stream = await self.node.stream_get("/live/base")
        with self.assertRaises(errors.ApiError):
            stream.name = "invalid"
            await self.node.stream_update(stream)
    ...
    async def testStreamCreateErrors(self):
        ...
        # missing elements
        stream = api.Stream(name="no elements", elements=[])
        with self.assertRaises(errors.ApiError):
            await self.node.stream_create(stream, "/test/f1/f1a")

        # invalid values
        stream = api.Stream(name="invalid datatype", datatype="invalid",
                            elements=[api.Element(name="e1")])
        with self.assertRaises(errors.ApiError):
            await self.node.stream_create(stream, "/test/f1/f1a")

    async def testStreamDelete(self):
        await self.node.stream_delete("/test/f1/f1a/s1a1")
        with self.assertRaises(errors.ApiError):
            await self.node.stream_get("/test/f1/f1a/s1a1")

    async def testStreamDeleteErrors(self):
        # missing stream
        with self.assertRaises(errors.ApiError):
            await self.node.stream_delete("/test/f1/f1a/bad")
    ...
    async def testStreamInfo(self):
        ...
        info = await self.node.stream_info("/archive/data2")
        self.assertEqual(info.total_time, 0)

    async def testStreamInfoErrors(self):
        # missing stream
        with self.assertRaises(errors.ApiError):
            await self.node.stream_info("/test/bad")
