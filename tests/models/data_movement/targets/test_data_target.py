...
import tempfile
from joule.models.data_movement.targets.data_target import DataTarget
...
class TestDataTarget(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_data_target(self):
        ...
        with(
            ...
            # list the contents of the work_path, should be 4 items
            ...
            with open(work_path + f"/{self.chunk1['timestamp'][-1]}.dat", 'rb') as f:
                data = np.load(f)
                np.testing.assert_array_equal(data, self.chunk1)

            with open(work_path + f"/{self.chunk2['timestamp'][-1]}.dat", 'rb') as f:
                data = np.load(f)
                np.testing.assert_array_equal(data, self.chunk2)

            # make sure the interval break file was created
            self.assertTrue(os.path.exists(work_path + f"/{self.chunk1['timestamp'][-1]+1}_interval_break.dat"))
